...
if result:
    print("He encontrado el patron del texto")
else:
    print("No he encontrado el patron en el texto") 

#.group() devuelve la cadena que coincide en el pattern
print(result.group())

#.star() devolver la posicion inicial de la coincidencia
print(result.start())
...
text ="Todo el mundo dice que la IA nos va a quitar el trabajo.Pero solo hace falta ver como la cagar con las regex para ir con cuidado"
...
text = "Me gusta Python. Python es lo máximo. Aunque Python no es tan difícil."
pattern="Python"
...
#EJERCICIO 02 
#Encuentre todas las ocurrencias de la palabra "midu" en
#el siguiente texto e indica en que posicion empieza y termina cada coincidencia
...
pattern= "midu"
matches = re.findall(pattern, text)
buscar_midu = re.search(pattern, text)
if buscar_midu:
    print(buscar_midu.group())
    print(buscar_midu.start())#posicion inicial
    print(buscar_midu.end())#posicion final
    print(len(matches))#cuantas veces se repite 


#Modificadores
#los modificadores son opciones que se pueden agregar a un patron 
#para cambiar su comportamineto

#re.IGNORECASE: ignora las mayusculas y minusculas

text ="Todo el mundo dice que la IA nos va a quitar trabajo.Pero la ia no es tan mala. ¡viva la ia!"

pattern="IA"
found= re.findall(pattern,text,re.IGNORECASE)
if found: print(found)
#EJERCICIO 03
# Encuentra todas las ocurrencias de la palabra "python" en el siguiente texto, sin distinguir entre mayúsculas y minúsculas. text = "Este es el curso de Python de midudev. ¡Suscríbete a python si te gusta este contenido! PYTHON"
### Reemplazar el texto
